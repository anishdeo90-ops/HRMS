...
import { normalizeExpenseClaimPayload, normalizeExpenseLineItems, sumExpenseLineItems } from "@/lib/hrms/expenses";
import { createClient } from "@/lib/supabase/server";

const CLAIM_SELECT = [
  "*",
  "employee:employees!expense_claims_employee_id_fkey(id, employee_code, name, profile_id, department_id, reporting_manager_id)",
...
export async function GET(req: NextRequest) {
  ...
  if (employeeId) {
    ...
    if (!canViewExpenseRecord(profile, employee) && !canApproveExpenseRecord(profile, employee, "expense_claim")) {
  ...
  let query = supabase.from("expense_claims").select(CLAIM_SELECT).order("created_at", { ascending: false }).limit(200);
  ...
  const scoped = (data ?? []).filter((record) => {
    ...
    return canViewExpenseRecord(profile, target) || canApproveExpenseRecord(profile, target, "expense_claim") || canManageExpenses(profile);
  });
...
export async function POST(req: NextRequest) {
  ...
  const items = normalizeExpenseLineItems(body.items);
  const payload = normalizeExpenseClaimPayload({ ...body, employee_id: employee.id });
  const { data, error } = await supabase
    .from("expense_claims")
    .insert({ ...payload, total_amount: sumExpenseLineItems(items), created_by: user.id })
    .select()
    .single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });

  if (items.length > 0) {
    const { error: itemError } = await supabase.from("expense_claim_items").insert(items.map((item) => ({ ...item, claim_id: data.id })));
