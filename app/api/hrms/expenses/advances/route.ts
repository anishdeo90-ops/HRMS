...
export async function GET(req: NextRequest) {
  ...
  if (employeeId) {
    ...
    if (!canViewExpenseRecord(profile, employee) && !canApproveExpenseRecord(profile, employee, "employee_advance")) {
  ...
  let query = supabase.from("employee_advances").select(ADVANCE_SELECT).order("created_at", { ascending: false }).limit(200);
  ...
  const scoped = (data ?? []).filter((record) => {
    ...
    return canViewExpenseRecord(profile, target) || canApproveExpenseRecord(profile, target, "employee_advance") || canManageExpenses(profile);
  });
...
export async function POST(req: NextRequest) {
  ...
  const payload = normalizeAdvancePayload({ ...body, employee_id: employee.id });
  const { data, error } = await supabase.from("employee_advances").insert({ ...payload, created_by: user.id }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
