...
import { buildLedgerEntry, normalizeEncashmentPayload } from "@/lib/hrms/leave";
import { canApproveLeaveEncashment, canManageLeaveBalances, canRequestLeaveEncashment, canViewLeave } from "@/lib/hrms/leave-authorization";
...
const ENCASHMENT_SELECT = "*,employee:employees!leave_encashments_employee_id_fkey(id, profile_id, department_id, reporting_manager_id)";
...
export async function PATCH(req: NextRequest) {
  ...
  const { data: existing, error: existingError } = await supabase.from("leave_encashments").select(ENCASHMENT_SELECT).eq("id", id).single();
  if (existingError) return NextResponse.json({ error: existingError.message }, { status: 500 });
  if (existing.status !== "submitted") return NextResponse.json({ error: "Only submitted encashments can be decided" }, { status: 400 });
  if (!canApproveLeaveEncashment(profile, target(existing))) return NextResponse.json({ error: "Insufficient permissions" }, { status: 403 });
  ...
  const { data, error } = await supabase.from("leave_encashments").update(patch).eq("id", id).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  ...
  if (action === "approve") {
    const entry = buildLedgerEntry({
      ...
      leave_period_id: existing.leave_period_id,
