...
import { normalizeVehicleLogPayload } from "@/lib/hrms/expenses";
import { createClient } from "@/lib/supabase/server";

const LOG_SELECT = [
  "*",
  "employee:employees!vehicle_logs_employee_id_fkey(id, employee_code, name, profile_id, department_id, reporting_manager_id)",
].join(",");
...
export async function GET(req: NextRequest) {
  ...
  if (employeeId) {
    ...
    if (!canViewExpenseRecord(profile, employee) && !canApproveExpenseRecord(profile, employee, "vehicle_log")) {
  ...
  let query = supabase.from("vehicle_logs").select(LOG_SELECT).order("created_at", { ascending: false }).limit(200);
  if (employeeId) query = query.eq("employee_id", employeeId);
  const { data, error } = await query;
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  const scoped = (data ?? []).filter((record) => {
    const target = targetFromRecord(record);
    return canViewExpenseRecord(profile, target) || canApproveExpenseRecord(profile, target, "vehicle_log") || canManageExpenses(profile);
  });
...
export async function POST(req: NextRequest) {
  ...
  const payload = normalizeVehicleLogPayload({ ...body, employee_id: employee.id });
  const { data, error } = await supabase.from("vehicle_logs").insert({ ...payload, created_by: user.id }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
