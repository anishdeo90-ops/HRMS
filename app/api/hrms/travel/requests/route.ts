...
const TRAVEL_SELECT = [
  "*",
  "employee:employees!travel_requests_employee_id_fkey(id, employee_code, name, profile_id, department_id, reporting_manager_id)",
...
export async function GET(req: NextRequest) {
  ...
  if (employeeId) {
    ...
    if (!canViewExpenseRecord(profile, employee) && !canApproveExpenseRecord(profile, employee, "travel_request")) {
  ...
  let query = supabase.from("travel_requests").select(TRAVEL_SELECT).order("created_at", { ascending: false }).limit(200);
  ...
  const scoped = (data ?? []).filter((record) => {
    ...
    return canViewExpenseRecord(profile, target) || canApproveExpenseRecord(profile, target, "travel_request") || canManageExpenses(profile);
  });
...
export async function POST(req: NextRequest) {
  ...
  const payload = normalizeTravelRequestPayload({ ...body, employee_id: employee.id });
  const dateRange = validateTravelDateRange(String(payload.start_date ?? ""), String(payload.end_date ?? ""));
  ...
  const { data, error } = await supabase.from("travel_requests").insert({ ...payload, created_by: user.id }).select().single();
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
  if (itinerary.length > 0) {
    const { error: itineraryError } = await supabase.from("travel_itineraries").insert(itinerary.map((item) => ({ ...item, travel_request_id: data.id })));
