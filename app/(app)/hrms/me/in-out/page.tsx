...
import { DEMO_MY_ATTENDANCE } from "@/lib/hrms/demo-data";
...
/**
 * `docs/hrms/04-me.md §1`.
 *
 * A row exists for every calendar day whether or not the employee punched —
 * this is the materialised day register, not a punch log grouped on read.
 * Weekly offs, holidays and absences need a row to be marked on.
 */

const WEEKDAY = ["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"];

/** `01-08-2026-Sat` — the reference's format, kept because it reads well here. */
function dateWithDay(iso: string): string {
...
export default function MyInOutPage() {
  ...
  const rows = useMemo(
    () =>
      DEMO_MY_ATTENDANCE.filter(
        (r) => r.work_date.startsWith(month) && (!status || r.day_status === status)
      ),
    [month, status]
  );

  const summary = useMemo(() => {
    const all = DEMO_MY_ATTENDANCE.filter((r) => r.work_date.startsWith(month));
    return {
      present: all.filter((r) => ["present", "wfh", "on_duty"].includes(r.day_status)).length,
      halfDays: all.filter((r) => r.day_status === "half_day").length,
      absent: all.filter((r) => r.day_status === "absent").length,
      leave: all.filter((r) => r.day_status === "on_leave").length,
      // Stored, never recomputed — the HRMS → Payroll contract (§6.2a).
      payable: all.reduce((sum, r) => sum + r.payable_fraction, 0),
      extra: all.reduce((sum, r) => sum + (r.extra_minutes ?? 0), 0),
    };
  }, [month]);
  ...
  const registerColumns: Column<AttendanceDay>[] = [
    ...
    },
    {
      ...
      align: "right",
      render: (r) => (
        <span className={r.payable_fraction === 1 ? "text-gray-700" : "font-semibold text-amber-700"}>
          {r.payable_fraction}
        </span>
      ),
    },
    {
      ...
      render: (r) =>
        r.penalty_reason ? (
          <span className="text-xs text-red-600">{r.penalty_reason}</span>
        ) : r.is_regularized ? (
          <span className="text-xs text-blue-600">Regularized</span>
        ) : (
          EMPTY
        ),
    },
  ...
  const punchRows = useMemo(
    () =>
      rows.flatMap((r) =>
        (r.punches ?? []).map((p) => ({ ...p, work_date: r.work_date }))
      ),
    [rows]
  );
  ...
  const punchColumns: Column<AttendancePunch & { work_date: string }>[] = [
    ...
    { key: "time", header: "Punch Time", render: (r) => fmtTime(r.punched_at) },
    {
      ...
      header: "Direction",
      render: (r) => (
        <Badge tone={r.direction === "in" ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600"}>
  ...
  return (
    <div className="space-y-5">
      <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <StatCard
          label="Extra Hours"
          value={fmtDuration(summary.extra)}
          tint="bg-gray-100 text-graphite"
        />
      </div>

      <Card title="My In Out" bodyClassName="p-4">
        <SubTabs
          tabs={[
            { key: "register", label: "Manual In-Out" },
            { key: "punches", label: "Multiple Punches", count: punchRows.length },
          ]}
          value={view}
          onChange={setView}
        />
        ...
        >
          ...
          >
            ...
            {["present", "half_day", "absent", "on_leave", "weekly_off", "holiday", "wfh", "on_duty"].map(
              (s) => (
