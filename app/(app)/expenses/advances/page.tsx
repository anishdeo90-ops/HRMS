...
import { Check, Paperclip, RefreshCw, Send, X } from "lucide-react";
...
export default function EmployeeAdvancesPage() {
  ...
  const [form, setForm] = useState({ requested_amount: "", request_date: "", purpose: "", notes: "" });
  ...
  async function load(nextStatus = status) {
    ...
    if (res.ok) setAdvances(await readList<AdvanceRow>(res));
      ...
      const message = (await res.json().catch(() => ({}))).error ?? "Could not load advances";
      setError(message);
      toast.error(message);
    }
    setLoading(false);
  ...
  async function submitAdvance() {
    if (!form.requested_amount || !form.request_date || !form.purpose.trim()) {
    ...
    setSaving("");
    if (!res.ok) {
      const message = (await res.json().catch(() => ({}))).error ?? "Advance request failed";
      setError(message);
      toast.error(message);
      return;
    }
    toast.success("Advance requested");
    setForm({ requested_amount: "", request_date: "", purpose: "", notes: "" });
    setAttachment(null);
    await load();
  }
      ...
      <div className="grid gap-5 xl:grid-cols-[360px_minmax(0,1fr)]">
        <div className="rounded-lg border border-gray-200 bg-white p-4">
          ...
          <div className="mt-3 grid gap-3">
            <input type="number" min="0" value={form.requested_amount} onChange={(event) => setForm({ ...form, requested_amount: event.target.value })} placeholder="Amount" className="rounded border border-gray-300 px-3 py-2 text-sm" />
            <input type="date" value={form.request_date} onChange={(event) => setForm({ ...form, request_date: event.target.value })} className="rounded border border-gray-300 px-3 py-2 text-sm" />
            ...
            <textarea value={form.notes} onChange={(event) => setForm({ ...form, notes: event.target.value })} placeholder="Repayment or settlement note" className="h-20 resize-none rounded border border-gray-300 px-3 py-2 text-sm" />
            <label className="flex items-center gap-2 rounded border border-dashed border-gray-300 px-3 py-2 text-sm text-gray-600">
              <Paperclip size={14} /> {attachment?.name ?? "Attach support file"}
              ...
              <div key={row.id} className="grid gap-3 px-4 py-3 xl:grid-cols-[minmax(0,1fr)_330px]">
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-medium text-gray-900">{money(row.requested_amount ?? row.amount)}</h3>
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${statusClass(row.status)}`}>{row.status ?? "draft"}</span>
                    <span className="text-xs text-gray-400">Required {row.request_date ?? row.required_by ?? "-"}</span>
                  </div>
                  <p className="mt-1 text-sm text-gray-600">{personLabel(row.employee)}</p>
                  <p className="mt-1 line-clamp-2 text-sm text-gray-500">{row.purpose ?? row.notes ?? "No purpose provided."}</p>
                </div>
