...
begin
  select a.reporting_manager_id, a.department_id, a.business_unit_id, p.role
    into v_manager_id, v_department_id, v_business_unit_id, v_requester_role
  from hrms.employees e
  left join public.profiles p on p.id = e.profile_id
  left join hrms.employee_assignments a on a.employee_id = e.id and a.is_primary and a.effective_to is null
  ...
  if v_requester_role in ('hod','manager','hr_manager') then
    v_approver_id := v_admin_id;
  ...
  v_second_approver_id := case
    when v_requester_role in ('hod','manager','hr_manager') then null
    when v_hr_manager_id is not null and v_hr_manager_id <> v_approver_id then v_hr_manager_id
    else v_admin_id
  end;

  if v_approver_id is null and v_second_approver_id is null then
    raise exception 'No approver configured for this request' using errcode = '22023';
  end if;
  ...
  from (
    ...
    from (
      ...
      union all
      select 'admin', v_hr_manager_id, 2
      where v_requester_role not in ('hod','manager','hr_manager')
        and v_hr_manager_id is not null
        and v_hr_manager_id <> v_approver_id
      union all
      select 'admin', v_second_approver_id, 3
    ) approvers
