...
begin
  ...
  if v_approver_id is null then
    v_approver_id := v_hr_manager_id;
    v_approver_source := 'admin';
  end if;

  if v_hr_manager_id is null then
    select e.id into v_hr_manager_id
    from hrms.employees e
    join public.profiles p on p.id = e.profile_id
    where e.org_id = v_org_id
  ...
  from (
    select v_approver_source as approver_source, v_approver_id as approver_id
    union
    select 'admin', v_hr_manager_id
  ) approvers
